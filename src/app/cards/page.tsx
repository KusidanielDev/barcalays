...
export default async function CardsPage() {
  ...
  async function toggle(id: string, current: "ACTIVE" | "FROZEN") {
    "use server";
    const u = await prisma.user.findUnique({ where: { email } });
    const card = await prisma.card.findFirst({ where: { id, userId: u!.id } });
    if (!card) return;
        ...
        <form
          action={async () => {
            ...
            await prisma.card.create({
              data: { userId: u!.id, accountId: acct.id, last4 },
            });
          }}
        >
        ...
        {cards.map((c) => (
          <div key={c.id} className="border rounded-2xl p-4">
            ...
            <div className="mt-3 flex gap-2">
              <form action={toggle.bind(null, c.id, c.status as any)}>
